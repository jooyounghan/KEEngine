...
namespace ke
{
	...
	void Texture2DResource::initialize(
		ID3D12Device* device,
		uint32 width,
		uint32 height,
		DXGI_FORMAT format,
		...
		uint16 mipLevels)
	{
		...
		KE_ASSERT(width > 0 && height > 0, "Texture dimensions must be greater than zero.");
		...
		// Create DEFAULT-heap 2D texture
		{
			...
			KE_ASSERT(!FAILED(device->CreateCommittedResource(
				...
				"Failed to create 2D texture default resource.");
		}
		...
		// Create UPLOAD-heap staging buffer
		{
			...
			KE_ASSERT(!FAILED(device->CreateCommittedResource(
				...
				"Failed to create 2D texture staging resource.");
		}
	...
	void Texture2DResource::shutdown()
	{
		releaseDescriptor();
	...
	void Texture2DResource::commitUpload(ID3D12GraphicsCommandList* commandList)
	{
